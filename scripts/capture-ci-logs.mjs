...
const report = { schemaVersion: '1.0.0', system: 'SOUL', generatedAt: new Date().toISOString(), repository, runId, state: 'DEGRADED', jobs: [], errors: [] };
...
if (!token || !repository || !runId) {
  ...
  try {
    const jobsPayload = await getJson(`/repos/${repository}/actions/runs/${runId}/jobs?per_page=100`);
    for (const job of jobsPayload?.jobs ?? []) {
      ...
      try {
        ...
        if (response.ok) {
          ...
          item.logLocationPresent = Boolean(location);
        }
      } catch (error) {
        item.logState = 'ERROR';
    ...
    report.state = report.jobs.length > 0 && report.jobs.every((job) => job.logState === 'CAPTURED') ? 'HEALTHY' : 'DEGRADED';
  } catch (error) {
