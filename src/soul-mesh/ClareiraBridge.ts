/**
 * Clareira application bridge for N02.
...
export async function forwardClareiraToN01(packet:ClareiraPacket):Promise<unknown>{
  ...
  const message={protocol:'soul-mesh/1',contractVersion:'1.1.0',id:globalThis.crypto?.randomUUID?.() ?? String(Date.now()),correlationId:packet.correlationId,source:'N02',target:'N01',kind:'request',capability:'clareira.ingest',payload:{packet},timestamp:Date.now(),meta:{runtime:'N02',transport:'HTTP',encoding:'json',version:'1.1.0',traceId:packet.correlationId}};
