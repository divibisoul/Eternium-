...
const ProgressBar: React.FC<{ value: number; color: string; icon: React.ReactNode }> = ({ value, color, icon }) => (
    <div className="flex items-center space-x-2 text-xs">
        <div className="text-gray-400">{icon}</div>
        <div className="w-full bg-gray-700/50 rounded-full h-1.5">
            <div className={`h-1.5 rounded-full ${color}`} style={{ width: `${value}%` }}></div>
        </div>
        <span className="font-mono-code text-gray-300 w-10 text-right">{value.toFixed(0)}%</span>
    </div>
);
