...
/**
 * Local neural-state adapter over the existing Soul Mesh.
 * The caller supplies the live Mesh endpoint; no parallel `/api/neural/federate`
...
export class SynapticNodeBridge {
  ...
  constructor(nucleusId: string, meshUrl: string, options: SynapticBridgeOptions = {}) {
    if (!/^N[1-7]$/.test(nucleusId) && !/^N0[1-7]$/.test(nucleusId)) {
      throw new Error(`Invalid SOUL nucleus id: ${nucleusId}`);
    }
  ...
  async sendState(state: LocalState): Promise<boolean> {
    ...
    try {
      const headers: Record<string, string> = {
        ...
        'X-Soul-Mesh-Correlation-Id': correlationId,
      };
      if (this.authorizationToken) headers.Authorization = `Bearer ${this.authorizationToken}`;

      const response = await fetch(this.meshUrl, {
        method: 'POST',
        headers,
      ...
      const body = (await response.json().catch(() => null)) as
        | { correlationId?: string; source?: string; target?: string; protocol?: string; contractVersion?: string }
        | null;

      if (!response.ok) return false;
      if (!body) return false;
      if (body.correlationId !== correlationId) return false;
      if (body.protocol !== 'soul-mesh/1' || body.contractVersion !== '1.1.0') return false;
      if (body.source !== 'N07' || body.target !== envelope.source) return false;
      return true;
    } catch (error) {
