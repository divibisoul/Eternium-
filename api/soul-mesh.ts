...
export default async function handler(req: any, res: any) {
  ...
  if (m.capability === 'mesh.ping') return res.status(200).json({ protocol: 'soul-mesh/1', id: crypto.randomUUID(), correlationId: m.correlationId, source: NUCLEUS_ID, target: m.source, kind: 'response', capability: 'mesh.ping', payload: { ok: true, handler: 'N02.mesh.ping', echoed: m.payload, processedAt: Date.now() }, timestamp: Date.now() });
  if (m.capability === 'mesh.describe') return res.status(200).json({ protocol: 'soul-mesh/1', id: crypto.randomUUID(), correlationId: m.correlationId, source: NUCLEUS_ID, target: m.source, kind: 'response', capability: 'mesh.describe', payload: { nucleus: NUCLEUS_ID, capabilities: ['mesh.ping', 'mesh.describe'], status: 'online' }, timestamp: Date.now() });
