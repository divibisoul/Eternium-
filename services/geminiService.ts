...
export interface AeternumGenerationResponse { text: string; candidates?: Array<{ groundingMetadata?: unknown }>; }
...
export const processUserDirective = async (mode: SystemAspect, contents: Content[], useWebSearch: boolean, deployedCapabilities: DeployedCapability[], isFullCognitionMode: boolean): Promise<AeternumGenerationResponse> => {
  ...
  try { return await callGemini('gemini-2.5-flash', contents, config); }
    ...
    if (error instanceof Error) {
      if (error.message.includes('AUTHENTICATION') || error.message.includes('API_KEY') || error.message.includes('permission')) throw new Error('[ERRO DE AUTENTICAÇÃO] A chave da API do núcleo é inválida, expirou ou carece de permissões.');
      if (error.message.includes('400') || error.message.includes('INVALID_')) throw new Error('[ERRO DE CONTEÚDO] A solicitação para o núcleo foi malformada.');
    }
...
export const transcribeAudio = async (audioBase64: string, mimeType: string): Promise<string> => {
  try {
    const response = await callGemini('gemini-2.5-flash', { parts: [{ inlineData: { mimeType, data: audioBase64 } }, { text: 'Transcreva o seguinte áudio para o português do Brasil. Responda apenas com o texto transcrito.' }] });
